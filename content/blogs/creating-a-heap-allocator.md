...
This is the raw data structre that will power our heap allocator. Each block is of 4 bytes. Now to keep the track of allocations and deallocations, we need to store some metadata along each block on the list. For this we'll attach a header and a footer to each block. The footer will be the copy of the header. A block will look like this:
Image
